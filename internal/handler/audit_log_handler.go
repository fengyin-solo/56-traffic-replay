...
func (s *Server) registerAuditLogRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("PUT /api/audit-logs/{id}", s.updateAuditLog)
...
func (s *Server) updateAuditLog(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	var req updateAuditLogRequest
	if err := httpx.Decode(r, &req); err != nil {
		httpx.BadRequest(w, "请求体解析失败: "+err.Error())
		return
	}
	a, err := s.svc.UpdateAuditLog(id, model.AuditLog{
		Operator:   req.Operator,
		Action:     req.Action,
		TargetType: req.TargetType,
		TargetID:   req.TargetID,
